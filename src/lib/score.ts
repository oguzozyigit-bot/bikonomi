// src/lib/score.ts
import type { MarketInfo, Offer, OfferVerdict, Score, Verdict } from "@/lib/types";

function clamp(n: number, a: number, b: number) {
  return Math.max(a, Math.min(b, n));
}

function getBestTotal(offers: Offer[]) {
  const totals = offers
...
/**
 * ✅ Teklif satırı etiketi (Mantıklı/Olur/Mantıksız)
 * /api/analyze bunu kullanıyor.
 */
export function computeOfferVerdict(params: {
  ...
  // 1) Stok yoksa direkt mantıksız
  if (!offer.inStock) return "Mantıksız";
  if (!Number.isFinite(total) || total <= 0) return "Mantıksız";

  // 2) Temel verdict (market varsa markete göre, yoksa best'e göre)
  ...
  // 4) Güven overrule (trustLevel varsa)
  // Not: Offer tipinde trustLevel var (0|1|2). Bazı offer'larda gelmeyebilir.
  const tl = (offer as any).trustLevel as 0 | 1 | 2 | undefined;
  if (tl === 0) {
    if (v === "Mantıklı") v = "Olur";
    else if (v === "Olur") v = "Mantıksız";
  }
...
export function computeScore(input: { offers: Offer[]; market: MarketInfo }): Score {
  ...
  const bestTotal = getBestTotal(offers);

  // En iyi offer = en düşük total (MVP)
  const bestOffer =
    offers
  ...
  const trustLevel = ((bestOffer as any).trustLevel as 0 | 1 | 2 | undefined) ?? 1;

  // 1) priceScore 0–40
  let priceScore = 20;
  if (marketAvg && marketAvg > 0) {
    const deltaPct = (marketAvg - bestTotal) / marketAvg;
  ...
  let marketScore = 10;
  if (marketAvg && marketAvg > 0) {
    const diff = (bestTotal - marketAvg) / marketAvg;
